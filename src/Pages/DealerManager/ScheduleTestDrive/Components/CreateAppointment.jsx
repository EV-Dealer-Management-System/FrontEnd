...
const CreateAppointmentForm = ({ onAppointmentCreated }) => {
    ...
    const fetchData = async () => {
      try {
        ...
        if (customersResponse.isSuccess) {
          ...
          message.error(customersResponse.message || 'Không thể tải danh sách khách hàng');
        }

        if (templatesResponse.isSuccess) {
          setTemplates(templatesResponse.result || []);
        } else {
          message.error(templatesResponse.message || 'Không thể tải danh sách template');
        }
      } catch (error) {
  ...
  const handleSubmit = async (values) => {
    try {
      ...
      const adjustedEndTime = validateAndAdjustTime(values.endDate);
      ...
      console.log('📤 Formatted Appointment Data:', JSON.stringify(formattedData, null, 2));
  ...
  return (
    <Card 
      ...
      extra={<Button type="link" icon={<PlusOutlined />}>Thêm</Button>}
    >
      <Form
        form={form}
        layout="vertical"
      ...
      >
        ...
        {selectedTemplate && (
          <div className="mb-4 p-3 bg-blue-50 border border-blue-200 rounded">
            <Text strong className="block mb-2">Chi Tiết Template</Text>
            <div className="grid grid-cols-2 gap-2">
              <div>
                <Text type="secondary" className="block">Phiên Bản:</Text>
                <Text strong>{selectedTemplate.version?.versionName}</Text>
              </div>
              <div>
                <Text type="secondary" className="block">Mẫu Xe:</Text>
                <Text strong>{selectedTemplate.version?.modelName}</Text>
              </div>
              <div>
                <Text type="secondary" className="block">Màu Sắc:</Text>
                <Text strong>{selectedTemplate.color?.colorName}</Text>
              </div>
              {selectedTemplate.imgUrl && (
                <div>
                  <Text type="secondary" className="block mb-1">Hình Ảnh:</Text>
                  <Image 
        ...
        >
          <Select placeholder="Chọn trạng thái">
            ...
            <Option value={2}>
              <span className="mr-2">⏳</span>Chờ Xác Nhận
            </Option>
            <Option value={3}>
              <span className="mr-2">🚫</span>Đã Hủy
            </Option>
            <Option value={4}>
              <span className="mr-2">✔️</span>Hoàn Thành
            </Option>
          </Select>
        </Form.Item>
