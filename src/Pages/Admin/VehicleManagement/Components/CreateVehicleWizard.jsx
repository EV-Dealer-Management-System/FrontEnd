...
function CreateVehicleWizard() {
  ...
  const handleCreateModel = async (values) => {
    try {
      ...
      console.log("=== DEBUGGING CREATE MODEL ===");
      console.log("Input values:", values);
      console.log("vehicleApi:", vehicleApi);
      console.log(
        "createModel function exists:",
        typeof vehicleApi.createModel === "function"
      );
      ...
      const result = await vehicleApi.createModel(modelData);
      ...
      if (result.success && result.data?.id) {
        ...
        message.success(result.message || "Tạo model thành công!");
        setCurrentStep(1);
      } else {
        console.error("Model creation failed:", result);
        message.error(result.error || "Lỗi khi tạo model - Không nhận được ID");
      }
    } catch (error) {
      console.error("Caught error in handleCreateModel:", error);
      console.error("Error message:", error.message);
      console.error("Error response:", error.response?.data);
      message.error(
        "Lỗi khi tạo model: " + (error.message || "Không xác định")
      );
    } finally {
      setLoading(false);
    }
  ...
  const handleCreateVersion = async (values) => {
    try {
      setLoading(true);
      const result = await vehicleApi.createVersion({
      ...
      if (result.success && result.data?.id) {
        ...
        message.success(result.message || "Tạo version thành công!");
        setCurrentStep(2);
      } else {
        console.error("Version creation failed:", result);
        message.error(
          result.error || "Lỗi khi tạo version - Không nhận được ID"
        );
      }
    } catch (error) {
      message.error(
        "Lỗi khi tạo version: " + (error.message || "Không xác định")
      );
    } finally {
      setLoading(false);
    }
  ...
  const handleCreateColor = async (values) => {
    try {
      ...
      console.log("=== DEBUGGING CREATE COLOR ===");
      console.log("Color form values:", values);
      console.log("Created data:", createdData);

      const colorData = {
        versionId: createdData.versionId,
        colorName: values.colorName,
        hexCode: values.colorCode, // Map colorCode to hexCode
        imageUrl:
          values.imageUrl ||
          "https://picsum.photos/400/300?random=" + Date.now(),
        additionalPrice: values.additionalPrice
          ? parseInt(values.additionalPrice)
          : 0,
      };

      console.log("Sending color data:", colorData);

      const result = await vehicleApi.createColor(colorData);
      ...
      if (result.success && result.data?.id) {
        ...
        message.success(result.message || "Tạo màu sắc thành công!");
        setCurrentStep(3);
      } else {
        console.error("Color creation failed:", result);
        message.error(
          result.error || "Lỗi khi tạo màu sắc - Không nhận được ID"
        );
      }
    } catch (error) {
      console.error("Error in handleCreateColor:", error);
      message.error(
        "Lỗi khi tạo màu sắc: " + (error.message || "Không xác định")
      );
    } finally {
      setLoading(false);
    }
          ...
          <Form form={modelForm} layout="vertical">
            ...
            <Row gutter={16}>
              <Col span={24}>
                ...
                >
                  <Input placeholder="VD: VF 8, Model 3, EQS" size="large" />
                </Form.Item>
              </Col>
                    ...
                    addonAfter={
                      <div
                        style={{
                          ...
                          height: 20,
                          backgroundColor:
                            colorForm.getFieldValue("colorCode") || "#e5e7eb",
      ...
      <Card>
        ...
        <div style={{ marginTop: 24, textAlign: "right" }}>
          <Space>
            ...
            {currentStep === 3 && (
              <Button onClick={handleReset}>Tạo xe mới</Button>
            )}
