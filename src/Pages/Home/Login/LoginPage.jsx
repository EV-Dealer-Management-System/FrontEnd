...
function LoginPage() {
  ...
  const handleLogin = async (values) => {
    ...
    try {
      ...
      // Kiểm tra nếu không có response (network error, server down, etc.)
      if (!err.response) {
        if (!navigator.onLine) {
          setLoginError(
          ...
          setLoginError(
            "Không thể kết nối đến server! Vui lòng thử lại sau hoặc liên hệ hỗ trợ."
          );
        }
        return;
      }
